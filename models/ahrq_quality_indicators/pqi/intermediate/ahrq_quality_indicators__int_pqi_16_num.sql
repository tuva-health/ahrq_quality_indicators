...
with diagnosis as (
    select distinct
        c.encounter_id
      , c.data_source
    from {{ ref('ahrq_quality_indicators__stg_pqi_condition') }} as c
...
, procedures as (
    select distinct
        p.encounter_id
      , p.data_source
    from {{ ref('ahrq_quality_indicators__stg_pqi_procedure') }} as p
...
from {{ ref('ahrq_quality_indicators__stg_pqi_inpatient_encounter') }} as e
inner join {{ ref('ahrq_quality_indicators__int_pqi_16_denom') }} as denom
...
left outer join {{ ref('ahrq_quality_indicators__int_pqi_16_exclusions') }} as shared
